...
#include "LongLongAttrHandler.h"
#include "Utility.h"

void CLongLongAttrHandler::WriteAttr(H5::DataSet& ElementDataSet, const Hdf5_Wrapper::DATA_ATTRIBUTE& AttributeElement)
{
	using namespace H5;
	DataType AttributeDataType(PredType::NATIVE_LLONG);
...
void CLongLongAttrHandler::WriteAttr(H5::Group& GroupNode, const Hdf5_Wrapper::DATA_ATTRIBUTE& AttributeElement)
{
	using namespace H5;
	DataType AttributeDataType(PredType::NATIVE_LLONG);
	DataSpace AttributeDataSpace(H5S_SCALAR);
...
void CLongLongAttrHandler::ReadAttr(H5::Attribute& Attr, H5::DataType& AttrDataType, std::vector<Hdf5_Wrapper::DATA_ATTRIBUTE>& AttributeArray)
{
	using namespace H5;
	//获取字节
	...
	if (H5S_SCALAR == AttrDataSpace.getSimpleExtentType())
	{
		...
		if (nullptr != pData)
		{
			...
			DataAttribute.eumatttype = Hdf5_Wrapper::DATA_TYPE::LONG_LONG_TYPE;
