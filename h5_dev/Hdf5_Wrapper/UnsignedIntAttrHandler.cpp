...
#include "UnsignedIntAttrHandler.h"
#include "Utility.h"

void CUnsignedIntAttrHandler::WriteAttr(H5::DataSet& ElementDataSet, const Hdf5_Wrapper::DATA_ATTRIBUTE& AttributeElement)
{
	using namespace H5;
	DataType AttributeDataType(PredType::NATIVE_UINT);
...
void CUnsignedIntAttrHandler::WriteAttr(H5::Group& GroupNode, const Hdf5_Wrapper::DATA_ATTRIBUTE& AttributeElement)
{
	using namespace H5;
	DataType AttributeDataType(PredType::NATIVE_UINT);
	DataSpace AttributeDataSpace(H5S_SCALAR);
...
void CUnsignedIntAttrHandler::ReadAttr(H5::Attribute& Attr, H5::DataType& AttrDataType, std::vector<Hdf5_Wrapper::DATA_ATTRIBUTE>& AttributeArray)
{
	using namespace H5;
	//获取字节
	...
	if (H5S_SCALAR == AttrDataSpace.getSimpleExtentType())
	{
		...
		if (nullptr != pData)
		{
			...
			DataAttribute.eumatttype = Hdf5_Wrapper::DATA_TYPE::UNSIGNED_INT_TYPE;
