...
CSmtp::CSmtp( int port, string srvDomain, /*smtp 衿饔蛎?*/ string userName, /*用 ?*/ string password, /*密码 */ const vector<string>& targetEmail, /*目的邮件地?*/ string emailTitle, /*主题 */ string content /*内容 */ )
{
	this->content = content;
	this->port = port;
	this->user = userName;
	this->pass = password;
	//this->targetAddr = targetEmail;
	m_ReceiverAddrList.assign(targetEmail.begin(),targetEmail.end());
	this->title = emailTitle;
	this->domain = srvDomain;
...
bool CSmtp::SendEmailHead()		//发送邮件头部信息
{
	...
	vector<string>::iterator Iter = m_ReceiverAddrList.begin();

	while (Iter != m_ReceiverAddrList.end())
	{
		//sendBuff = "RCPT TO: <" + targetAddr + ">\r\n";
		sendBuff = "RCPT TO: <" + *Iter/*targetAddr*/ + ">\r\n";
		if (false == Send(sendBuff) || false == Recv())
		{
			return false; /*表示发送失败由于网络错误*/
		}
...
{/*格式化要发送的内容*/
	...
	for (int i = 0; i < (int)m_ReceiverAddrList.size(); ++i)
	{
		if (i == m_ReceiverAddrList.size() - 1)
		{
			email += ("<" + m_ReceiverAddrList[i] + ">");
		}
		else
		{
			email += ("<" + m_ReceiverAddrList[i] + ">,");
...
int CSmtp::SendAttachment_Ex() /*发送附件*/
{
	for (list<FILEINFO *>::iterator pIter = listFile.begin(); pIter != listFile.end(); pIter++)
	{
		...
		//cout << ifs.gcount() << endl;
		chSendBuff = base64Encode(fileBuff, static_cast<unsigned int>(ifs.gcount()));
		chSendBuff[strlen(chSendBuff)] = '\r';
		chSendBuff[strlen(chSendBuff)] = '\n';
